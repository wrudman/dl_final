...
def get_missing(file_path):
	...
	missing_nums = []
	for i in range(max(clean_signals)):
...
def make_spec(windows, target_directory, ecg_type):
	for i in range(17597, len(windows)):
...
def main():
	windows = get_data(file_path="/gpfs/data/ceickhof/ecg_data/data/nm_data/files")
	make_spec(windows, target_directory="/gpfs/data/ceickhof/ecg_data/data/nm_spec", ecg_type="nm")
	#print(len(get_missing("/gpfs/data/ceickhof/ecg_data/data/nm_spec")))	
