...
def get_missing(file_path):
	...
	for files in glob.glob("*.png"):
		file_list.append(files.replace('nm','').replace(".png",''))
	clean_signals = []
	for i in range(len(file_list)):
		f = int(file_list[i])
		clean_signals.append(f)
	missing_nums = []
	for i in range(17597):
		if i not in clean_signals:
...
def get_data(file_path, target_directory, ecg_type):
	...
	missing_nums = get_missing(target_directory)	
	split = int(np.floor(len(missing_nums)/2))	
	missing_nums = missing_nums[split:]		
...
def main():
	get_data(file_path="/gpfs/data/ceickhof/ecg_data/data/nm_data/files", target_directory="/gpfs/data/ceickhof/ecg_data/data/nm_spec", ecg_type="nm")
	#print(get_missing("/gpfs/data/ceickhof/ecg_data/data/nm_spec"))	
